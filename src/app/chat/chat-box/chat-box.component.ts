...
//on pressinh any key this will be clalled
  ...
  //loads all the group and diffrentiates between joined and not joined
  public groupLoad = () => {

    this.AppService.getAllGroups().subscribe((apiResponse) => {
      this.otherGroups = [];
      this.groupsJoined = [];
      if (apiResponse.status == 200) {
  ...
  public deleteGroup = () => {
    ...
    data.userEmail = this.userInfo.email;
    data.roomId = this.receiverId;
    this.AppService.deleteGroup(data)
      .subscribe((apiResponse) => {
        if (apiResponse.status == 200) {
...
public getMessageFromGroup: any = () => {
